// ==UserScript==
// @name         Panopto-Video-DL-Tripos
// @namespace    https://github.com/Panopto-Video-DL-Tripos
...
// @author       Panopto-Video-DL, Tripos-Download
// @version      1.0
  ...
  if (location.pathname.includes('/List.aspx')) {
    $(document).ready(function() {
      if (location.pathname.includes('/List.aspx')) {
            ...
            .then(responses => {
              ...
              responses.forEach(response => {
                if (response.status == 'fulfilled' && response.value) {
                  const streams = response.value?.[1];
                  const streamUrl = response.value?.[0];
                  copyText += 'Audio\n' + streamUrl + '\n';
                  streams.forEach((value, index) => {
                    if (!(/Logo*./).test(value.Name)) {
                      copyText += value.Name + '\n';
                      copyText += value.StreamUrl + '\n';
                    }
                  })
                }
              });
        ...
        document.querySelectorAll('#actionHeader button')[1]?.parentElement.appendChild(button);
      }
    });
  }
